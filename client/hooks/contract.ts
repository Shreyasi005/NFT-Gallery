...
export const CONTRACT_ADDRESS =
  "CCJQIJLSLJ6PPIWJHUSGJMB7WIZCZ66LNWT56PAKH6T26XQYYPOVFXUY";
...
/**
 * Mint a new NFT.
 * Calls: mint(owner: Address, name: String, uri: String)
 */
export async function mintNFT(
  ...
  return callContract(
    "mint",
    [toScValAddress(caller), toScValString(name), toScValString(uri)],
    caller,
    true
  );
}
...
/**
 * Get NFT by ID (read-only).
 * Calls: get_nft(id: u32) -> Option<NFT>
 * Returns: { owner: string, name: string, uri: string } or null
 */
...
/**
 * Get all NFTs (read-only).
 * Calls: get_all() -> Vec<NFT>
 * Returns: Array of { owner: string, name: string, uri: string }
 */
